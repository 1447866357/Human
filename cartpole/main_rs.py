    ...
    for l in L:
        for r in R:
            for c in C:
                ...
                agent = rs.RewardShapingAgent(env,
                                              os.path.join(model_base_path, 'base', 'ckpt.ckpt'),
                                              os.path.join(model_base_path, 'feedback', 'control_sharing',
                                                           'model.ckpt'), l, r, c)
                ...
                for i in range(runs):
                    ...
                    pd.DataFrame(data).to_csv(
                        '/Users/d/work/Human/CartPole_OLD/data/reward_shaping/data_' +  str(l) + '_'+ str(c) + '_'+ str(r) + '_' +  str(i) + '_'  + str(MAX_EPISODE) + '.csv', header=dict)
...
#                     pickle.dump(total_reward, open('pic/feedback/reward_shaping/data_%.1f_%.1f_%.1f.pkl' % (l, r, c), 'wb'))
#                 agent.close()
