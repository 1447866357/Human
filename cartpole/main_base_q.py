import gym, os, BaseQLearningAgents as bq
import numpy as np
    ...
    for l in L:
        for r in R:
            for c in C:
                ...
                agent = bq.BaseDeepQLearning(env,   os.path.join(model_base_path, 'base', 'ckt.ckpt'))
                                                                                            # os.path.join(model_base_path, 'feedback', 'control_sharing', 'model.ckpt'), l, r, c)
                ...
                for i in range(runs):
                    ...
                    pd.DataFrame(data).to_csv(
                        '/Users/d/work/Human/CartPole_OLD/data/base_q/data_' +  str(l) + '_'+ str(c) + '_'+ str(r) + '_' +  str(i) + '_'  + str(MAX_EPISODE) + '.csv', header=dict)
...
#             for c in C:
#                 env = gym.make(ENV_NAME)
#                 agent = bq.BaseDeepQLearning(env,
#                                                os.path.join(model_base_path, 'base', 'ckt.ckpt'))
#                                                # os.path.join(model_base_path, 'feedback', 'control_sharing', 'model.ckpt'), l, r, c)
#
#                 for i in range(3):
#                     episode_reward = main()
...
#                     pickle.dump(total_reward, open('pic/feedback/base_q/data_%.1f_%.1f_%.1f.pkl' % (l, r, c), 'wb'))
#                 # agent.close()
